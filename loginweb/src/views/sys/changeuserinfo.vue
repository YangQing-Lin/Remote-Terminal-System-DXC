...
export default {
    data() {
      ...
      var validatephone = (rule, value, callback) => {
          var regex = new RegExp("^((13[0-9])|(14[579])|(15([0-3]|[5-9]))|(16[56])|(17[0-8])|(18[0-9])|(19[1589]))\\d{8}$");
          if (value === '') {
  ...
  computed: {
    ...
    methods: {
      ...
      beforeAvatarUpload(file) {
        ...
        this.imageUrl = "https://project-static-file.oss-cn-hangzhou.aliyuncs.com/avatar/" + file.name;
        this.userForm.url = this.imageUrl;
        // store.commit('user/SET_AVATAR', this.imageUrl);
        console.log("store avatar: ", store.state.user.avatar, this.userForm.url);
        // return isJPG && isLt2M;
        return false;
      },
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
              ...
              userApi.updateUser(store.state.user.token, this.userForm).then(response => {
