      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
            this.$confirm('是否确认修改密码?', '提示', {
                ...
                console.log(store.state.user.token, store.state.user.username, this.ruleForm2.oldpass, this.ruleForm2.pass);
                userApi.updatePassword(store.state.user.token, store.state.user.username, this.ruleForm2.oldpass, this.ruleForm2.pass).then(response => {
