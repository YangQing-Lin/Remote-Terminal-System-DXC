    ...
    <el-dropdown>
      ...
      <el-dropdown-menu slot="dropdown">
        <div v-for="(item, index) in convList" :key="index">
          <el-dropdown-item @click.native="toConv(index)">{{ item.name }}</el-dropdown-item>
        </div>
      </el-dropdown-menu>
    </el-dropdown>
    <div v-loading="isloading" :element-loading-text="waitingtext">
      <el-tabs v-model="editableTabsValue" type="card" @tab-click="handleClick" closable @tab-remove="removeTab" v-if="isShow">
        <el-tab-pane
  ...
  methods: {
    ...
    init() {
      ...
      setTimeout(() => {
        ...
        this.editableTabsValue = this.$store.state.other.terminalInfo.length + ''
    ...
    getIndication() {
      ...
      this.timer = setInterval(() => {
        ...
        this.editableTabs.forEach((item, index) => {
          ...
          item.titlename = (this.colorIndication_1()[index]===undefined?'⚪':this.colorIndication_1()[index]) + item.title
        })
      }, 1000);
    },
    addTab(tabInfo) {
